...
const HARMLESS_RESIDUAL_TOKENS = new Set([
  ...
  // 기존 커밋 보장 어휘(HIGH-1/T-1 회귀 계약 — "당도 8"/"점수 8"은 커밋 유지)
  '당도', '점수',
  // 🔴 v0.51.1 R3(2026-09-02 실기기 · STT 레인 §5 A3) — **'다시'는 여기서 뺐다.** 종전엔 「다시 점수 8」
  //   픽스처 계약으로 '다시'를 벗기고 값을 커밋했는데, 실발화에서 「다시 <값>」은 **재발화·재청취 신호**다:
  //   전 회차 값 동반 「다시」 4건(09-02 「다시 1.4」→1.4 오커밋 · 09-01 「다시 5.3」·「다시 오」 오커밋 ·
  //   정답 1) 중 오커밋 3. 「다시 점수 8」 형태는 09-01·09-02 로그에 0건(픽스처였지 실발화가 아니다).
  //   이제 「다시 <값>」은 `extraneous_token` 재질문이다(단독 「다시」는 종전대로 명령이 아니라 거절).
  //   대가: 소수부 재질문 문맥의 「다시 점 팔」(양승보 r14 · 40.8 정답 커밋)이 재질문으로 바뀐다.
  //   오라클: tests/koreanNum.spec.ts R3(픽스처 반전 2건 + 「다시 1.4」).
]);
