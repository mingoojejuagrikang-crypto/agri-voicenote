...
export const VOICE_COMMANDS: CommandSpec[] = [
  ...
  { id: 'screenOff',           word: '화면',             display: '화면 끄기',        desc: '화면을 꺼서 배터리를 아낍니다. 음성 입력은 계속됩니다 — 화면 가운데를 2초 누르면 다시 켜집니다',
    // 🔴 v0.51 [CLIP-MUTED-SPAN-1] — **이 문장은 상태를 따르지 않는다(의도된 예외).**
    //   같은 문구가 절전 화면(`BlackoutOverlay`)·홀드 안내(`HeroHoldToBlackout`)에도 있고
    //   그 둘은 트랙 `muted`일 때 「⚠ 마이크가 멈춰…」로 바뀌지만, **여기는 도움말의
    //   「이 명령이 무엇을 하는가」** 이지 현재 상태 고지가 아니다. 상태로 흔들면 도움말이
    //   상황마다 달라져 읽는 사람이 **기능 자체를 오해한다**(「어떤 땐 녹음이 안 되는 명령인가?」).
    //   👉 상태 고지는 그 순간의 화면이 지고, 여기는 기능 설명으로 남는다.
