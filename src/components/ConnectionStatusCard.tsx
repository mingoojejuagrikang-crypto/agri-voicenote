 ...
 *     구독으로 갱신 → stale 표시 해소.
 *     v0.55.0 결정 1-b(민구 09-17 「설정화면에서 그린인데 실제로 로그인 안된 경우가 종종」) — 🟢 토큰 유효(남은 분) / 🟡 토큰 없음·4주 창 살아 있음(재인증 필요 · [탭해서 갱신]) / 🔴 둘 다 없음. v0.51의 「창 = 연결」 표시는 이 결정으로 바뀌었다.
...
export function ConnectionStatusCard() {
  ...
  // 1) Google 연결 — v0.55.0 결정 1-b(민구 09-17 「설정화면에서 그린인데 실제로 로그인 안된 경우가 종종」) —
  //    🟢 토큰 유효(남은 분) / 🟡 토큰 없음·4주 창 살아 있음(재인증 필요 · [탭해서 갱신]) / 🔴 둘 다 없음.
  //    v0.51의 「창 = 연결」 표시는 이 결정으로 바뀌었다. (30s 폴링 틱이 만료 순간도 그대로 잡는다.)
