...
//   402×874(sab 0) + 375×667(sab 34)에서 검증. z-index 단언은 하지 않는다 — 나비와 시트가
//   공간상 안 겹치므로 z 순서는 무의미하고, z만 보면 잘림을 놓친다(geometry가 진짜 오라클).
//   ⚠️ **종전 주석은 그 비겹침의 원인을 `bottomInset`으로 적었는데 거짓이었다**(2026-08-31 실측
//   정정): 이 시트는 오버레이가 아니라 **그리드 인라인**이라 애초에 나비 자리를 안 먹는다.
//   `ModalBase.bottomInset`의 호출부는 v0.51 P0-1 이전까지 **0건**이었다.
for (const vp of [
