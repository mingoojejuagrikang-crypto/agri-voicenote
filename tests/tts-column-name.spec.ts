...
test.describe('formatNameForTts — 배수 꼬리 x<숫자>를 읽지 않는다 (v0.51.1 B3)', () => {
  ...
  // 🔴 v0.52(민구 결정 09-03) — **여기가 뒤집혔다.** 09-01 시점의 이 단언은 *"STT 매칭은 원문
  //   위에 그대로다 — 「수정 과피두께」로는 지목되지 않는다"* 였고, 그 아래 §「STT 매칭 불변」의
  //   주석이 *"민구가 STT도 축약본을 받게 하기로 정하면 이 줄이 가장 먼저 red가 되어야 한다"*
  //   고 적어 두었다. 그 결정이 09-03에 났다. **원문 지목은 그대로 살아 있다**(더 구체적인
  //   발화라 죽이지 않는다) — 바뀐 것은 축약형이 **추가로** 먹힌다는 것뿐이다.
  test('v0.52 — 축약형 지목이 먹힌다(원문 지목은 그대로)', () => {
    expect(extractModifyColumn('수정 과피두께x4', ['과피두께x4', '당도'])).toBe('과피두께x4');
    expect(extractModifyColumn('수정 과피두께', ['과피두께x4', '당도'])).toBe('과피두께x4');
  });
});
...
test.describe('STT 매칭 — 원문 지목은 불변, 축약형이 추가된다(v0.52)', () => {
  // 🔴 09-01 변경은 **TTS 출력 경로에만** 걸었다. v0.52가 그 간극(귀로 배운 이름을 말하면 지목 실패)을 닫는다.
  test('원문 이름 지목은 종전대로 먹힌다', () => {
    expect(extractModifyColumn('수정 종경(mm)', ['횡경(mm)', '종경(mm)'])).toBe('종경(mm)');
  });
  // 🔴 v0.52(민구 결정 09-03) — **예고된 대로 뒤집혔다.** 09-01 이 자리의 주석은 *"민구가 STT도
  //   축약본을 받게 하기로 정하면 이 줄이 가장 먼저 red가 되어야 한다"* 였다. 그 결정이 났고
  //   이 줄이 정확히 그 red였다. 실측 귀결이 「지목 실패」가 아니라 **두 칸 소거 + 엉뚱한 칸 열기**
  //   였다는 것이 처방의 긴급도였다(콜드 리뷰 §4 P1②).
  //   상세 계약(한 칸만·비파괴·모호)은 `tests/v052-modify-column-spoken.spec.ts`가 소유한다.
  test('v0.52 — 앱이 가르친 축약본으로 지목된다', () => {
    expect(extractModifyColumn('수정 종경', ['횡경(mm)', '종경(mm)'])).toBe('종경(mm)');
  });
});
