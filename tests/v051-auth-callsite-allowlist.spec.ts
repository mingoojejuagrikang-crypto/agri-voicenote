...
const ALLOWED: Record<string, string> = {
  ...
  'src/lib/syncAuthGuard.ts':
    '동기화·업로드 경로의 세션 활성 가드 그 자체 — 세션 live면 갱신 없이 false(r2). r4에서 useDataActions에서 분리했고 가드 술어를 이 파일이 소유한다 · 연결 상태 카드 [탭해서 갱신] 로그인 창의 signIn(사용자 명시 의사 · 세션 live면 no-op — v0.55.0)',
