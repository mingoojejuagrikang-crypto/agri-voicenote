...
- **판정 기준(다음 프리뷰 실사용 로그):** ⓐ `stt_confusion_hint` 발동 수와 `chosen` 분포(heard/alt/respoken/-) · ⓑ 질문이 난 셀의 최종값이 후보와 같았던 비율(시뮬 LOSO · r2: 발동 20 · 포착 12 · 오답후보 8) · ⓒ clean 셀 위양성(시뮬 0 — 실기기에서 1건이라도 나오면 `theta`/`kSupport`를 손보고 `tests/sttConfusion-clean-fixture.spec.ts`를 갱신) · ⓓ 「첫째」 뒤 같은 규칙의 질문이 줄어드는지(프로필 `negatives`).
- **알려진 한계:** 콜드스타트(09-01 표만으로 09-02 평가)는 발동 0 — 표는 같은 컬럼 종류의 데이터가 쌓여야 말한다. 소수부 자리 치환·두 자리 이상 다른 쌍(`multi_diff`)·앞자리 유실은 후보를 만들지 않는다(기록만). 정수(개수) 컬럼에는 묻지 않는다(r2 P2-3 — 09-02 오커밋 64건은 전부 소수 컬럼이고 개수 컬럼은 1·2·3이 정상값). 답변 어휘는 순서 낱말(첫째/둘째/셋째 · 1번/2번/3번 · 앞/뒤)과 네/아니오뿐이며 맨 숫자·수사(일/이/삼 · 하나/둘/셋 · 1/2/3)는 값 재발화(소수부 조각 포함)로 파서에 간다. 「둘째」 재커밋의 `value.text`는 답변 낱말이고 원 STT 원문은 첫 커밋 `value`와 `stt_correction`에 있다. 질문이 걸린 셀에 **손으로** 값을 넣으면 질문은 그 자리에서 `chosen=-`로 결산된다(늦게 남는 게 아니다). `exportLogZip`의 `speakerId`·`stt-profile.json` 동봉 배선은 순수 `attachSttProfile` 스펙만 있고 zip 다운로드 e2e는 없다(다음 회차 zip 수확에서 확인).
...
- **현재 상태:** 🟡 MONITORING — 데스크톱 e2e·시뮬만. 실기기 판정 전까지 RESOLVED 아님. **v0.52.0(2026-09-17) 릴리스에 포함** · 09-16 프리뷰 5세션에서 `stt_confusion_hint` 12건(세션별 3/3/2/1/3 · `beep_play:kind=alert` 12와 일치) 발생 — 질문→답→최종값 추적은 teamops logread 2단계 대기.
...
- **현재 상태:** 🟡 **MONITORING** — (아래 단락은 v0.51.0 시점 기술이다 · 09-02 실기기 1차 판정은 🔎, 그 처방은 🩹 블록)
  **2026-09-17:** 🩹 처방(ⓓ 판정 유예)은 **v0.52.0 릴리스에 포함**됐다 — 아래 🩹 머리의 「브랜치 · 미배포」는 당시 상태다. **2차 판정: ⚪ 미판정** — 09-16 프리뷰 5세션 `mic_interrupt_ui`·`mic_interrupt:*`·`mic_interrupt_notice`·`mic_track_evt` 전부 0(끼어들기 없음). 방출 코드는 `d8411eb:src/lib/micInterruption.ts:46·52`에 살아 있고 09-15 정식은 `mic_interrupt` 12건을 남겼다 → 계측 사망이 아니라 표본 0. 아래 판정 조건 그대로 유지(teamops `logread-B-quick.md`).
...
- 🔎 **실기기 판정 1차 (2026-09-02 · 프리뷰 실사용 `sess_1788316707658` · 품질조사 18행 · iPhone iOS 18.7 · 코드 = v0.51.0):**
  ...
  · ⓓ 🔴 **실패 — 배선 결함(구조적).** `mic_interrupt_notice` 0건 · 회복 TTS 0건. 원인: `useMicInterruptionNotice.ts`의
    ...
    ⚠️ 단, 두 건 다 값은 멀쩡히 잡혔다(`47.8`·`57` · conf 0.99 · STT 결과가 unmute 뒤 도착).
    🟢 **민구 결정 (2026-09-02): (a) unreliable이면 무조건 고지** — (b) 담겼으면 침묵 / (c) 절전 중만 은 기각.
...
> **현재 상태(2026-09-17):** 이 절 전부 **v0.52.0 릴리스에 포함**(머리의 「브랜치」는 당시 상태). 09-16 프리뷰 1단계 판독(teamops `logread-B-quick.md`): 배포 막힘 0 · 새 계측 `sheet_synced` 5·`start_ready` 3 관측 · 아래 판정 항목 ①~⑧의 효과는 2단계 분석 대기 — `MONITORING` 유지.
...
- **제보③ 「과피두께 TTS 42.2 · 칩 12.2」(15:54 양훈성 r2) — B3.** 열 이름 꼬리 `x4`가 수정 확인 TTS 「수정 과피두께x4 12.2」에서
  ...
  「수정 과피두께」로는 종전처럼 지목되지 않는다 — **당시 기준. v0.52에서 바뀌었다:** 앱이 읽어주는 이름으로 지목된다 · 아래 「2026-09-03 v0.52」 절). 특성화 스펙 4건이 쉼표로 갱신됐다(의도된 동작 변경 · PRINCIPLES §7 —
...
## 2026-09-03 v0.52 — 음성 「수정 <값>」 저장 확인(teamops `DEF-003`) · 「수정 <이름>」 지목 (빌드 SSOT: `workspace_teamops/deliverables/2026-09-03-v052-round/L1-def003-build.md` · 브랜치 `feat/v052-round` `d8411eb` → v0.52.0 정식)

> 09-03 빌드 · 콜드 리뷰 P1 2건 소비 · 09-16 프리뷰 실사용 · **v0.52.0(2026-09-17) 릴리스에 포함.** 음성·저장 경로 변경이라
> 실기기 판정 전까지 전부 `MONITORING`이다(AGENTS.md 계약 ④). 아래는 09-02 절과 같이 **ID 없이** 서술한다.

- **무엇 ① 음성 「수정 <값>」 거짓 성공(`DEF-003`, 08-24 배포본 점검에서 발견).** `enterModifyMode`의 직접값 분기가 persist 두 갈래
  (`saveSession`·`persistSession`)를 fire-and-forget으로 버려, IDB 쓰기가 실패해도 ✓·착지 재안내가 무조건 나갔다(IDB엔 옛 값 · 새로고침 시 유실).
  처방: 두 갈래 결과를 받고 **`loadSession` 되읽기로 착지를 확인**(반환값만 믿지 않는다 — `persistSession`은 쓰지 않고 `true`를 돌릴 수 있다) →
  ✓(`useSessionCommitMarks.add`)·착지 재안내가 durable을 따른다 · 실패면 셀 배너 + [다시 저장]. **확인음·에코 TTS는 종전 자리**(민구 계약).
  이상치 알람 분기도 알람 TTS 뒤에 정산한다. 실패 사유는 기존 `cell_persist_failed:` 메시지 슬롯(`session_not_durable`·`row_unlanded`·`read_failed:`).
- **무엇 ② 「수정 <이름>」.** `matchModifyColumn`이 원문 키 + `formatNameForTts` 키(앱이 읽어준 이름)로 매칭 · 지목(`single`)은 **한 칸만** 비운다
  (종전 reviewWait 지목은 그 열부터 행 끝까지 소거) · 미매칭은 **비파괴 착지**(그 국면의 기존 대기 문구 — 새 TTS 문구 0) · 축약형이 같은 열
  2~3개면 `modifyColumnConfirm` 대기 상태로 「첫 번째 X인가요, 두 번째 X인가요?」(4개 이상은 묻지 않음). 질문 중 `cellScoped` 명령
  (수정·취소·유지·확인)은 답변 게이트로 간다(콜드 리뷰 P1-1 — 다른 행 소거·덮어쓰기). 적용 국면은 reviewWait·cellWait·atEnd뿐이다.
- **판정 기준(09-16 프리뷰 로그 · 이후 정식 로그):** ⓐ 「수정 <값>」 뒤 앱 재시작·재개에도 값이 남는가(`cell_persist_failed` 발생 시 배너·재시도 회수)
  ⓑ `modify_target_unmatched:<국면>` 뒤 **어느 칸도 비지 않았는가** · 지목 뒤 **한 칸만** 바뀌었는가 ⓒ `modify_column_confirm:<n>` 도달(표본 0이면 미판정 —
  실 시트는 축약형 충돌 0이 예상된다). 판정 전까지 `MONITORING`.
  **09-16 프리뷰 1단계 판독(teamops `logread-B-quick.md` · 품질조사 5세션):** 배포 막힘 0. ⓐ 정상 저장 갈래 🟢 — 「수정 <값>」 27/27 되읽기 확인 · 실패 0 · 최종값 27/27 · 확인음·에코 지연 없음(중앙 2ms) / 재시작 후 유지 ⚪(재시작 0회) / 실패 배너·[다시 저장] ⚪(실패 0회)
  ⓑ ⚪(항목명 지목 발화 0회) ⓒ ⚪(도달 불가 — 품질조사 시트 축약형 충돌 0). 🔑 실기기가 밟은 v0.52 경로는 DEF-003 정상 저장 갈래 하나뿐 — 나머지는 여전히 데스크톱 오라클이 유일한 근거다.
- **남긴 것(빌드 SSOT §9-4 — 고치지 않은 이유가 있다):** ① hold 갈래(`commitManualValue` 알람 보류)의 `durable && !landed`에 배너를 세우지 않는다
  — 도달로 미발견 · 롤백 의미론과 얽혀 민구 확정 필요 ② 질문 중 저신뢰 「수정」(<0.40)은 `rejectLowConfidence` 꼬리가 `relistenPrompt(name)`로 떨어진다
  (파괴 없음) ③ 질문 중 「취소」는 `modify_column_cancelled:not_an_answer`로 기록된다(동작은 「아니오」와 같음 · 어휘 표가 R6와 공유라 둠)
  ④ 알람 값의 [다시 저장] 뒤 같은 알람 TTS가 한 번 더 나고 알람이 `manualHold:true`(터치 전용)로 다시 선다 — 🔴 이상치 알람은 기능 동결(민구 09-08)이라
